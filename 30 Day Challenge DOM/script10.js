...
let template = document.querySelector(".block");

const namesEl = document.querySelector(".lang-names");

const firstTen = countries_data
  .sort((a, b) => b.population - a.population)
  .slice(0, 10);

const sum = countries_data.reduce((acc, curr) => acc + curr.population, 0);

function countryNames() {
  let totalSum = document.createElement("div");
  totalSum.classList.add("block");
  let total = `
  <div class = "names">
            <div class="lang-names">World</div>
          </div>
          <div class="bar">
            <div class="bar-display"></div>
          </div>
          <div class="numbers">
            <div class="actuals">${sum}</div>
          </div>  
  `;
  totalSum.innerHTML = total;
  data.appendChild(totalSum);
  for (i = 0; i < firstTen.length; i++) {
    let barWidth = (firstTen[i].population / sum) * 100;
    let block = document.createElement("div");
    block.classList.add("block");

    template = `
          <div class="names">
            <div class="lang-names">${firstTen[i].name}</div>
          </div>
          <div class="bar">
            <div class="bar-display" style="width: ${(barWidth * 2).toFixed(
              1
            )}%"><div class="bar-text">${barWidth.toFixed(1)}%</div></div>
          </div>
          <div class="numbers">
            <div class="actuals">${firstTen[i].population}</div>
          </div>
    `;
...
const langArr = countries_data.map((country) => country.languages);

let langFreq = langArr.reduce((acc, currVal) => {
  acc[currVal] = (acc[currVal] || 0) + 1;
  return acc;
}, {});

let totalLanguages = Object.values(langFreq).reduce(
  (acc, currVal) => acc + currVal,
  0
);

let sortedFreqArr = Object.entries(langFreq)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 10);

// console.log(sortedFreqArr);

function countryLanguages() {
  for (i = 0; i < sortedFreqArr.length; i++) {
    let barWidth = (sortedFreqArr[i][1] / totalLanguages) * 100;
    let block = document.createElement("div");
    block.classList.add("block");

    template = `
          <div class="names">
            <div class="lang-names">${sortedFreqArr[i][0]}</div>
          </div>
          <div class="bar">
            <div class="bar-display" style="width: ${(barWidth * 2).toFixed(
              1
          ...
          <div class="numbers">
            <div class="actuals">${sortedFreqArr[i][1]}</div>
          </div>
    `;
    block.innerHTML = template;
    data.appendChild(block);
