...
function filterArr(letter) {
  letter = userEntry.value.toLowerCase();

  let countryName = countries_data.filter((country) =>
    country.name.toLowerCase().startsWith(letter)
  );

  countryName.forEach((country) => {
    let fiteredDiv = document.createElement("div");
    fiteredDiv.className = "box1";
    fiteredDiv.style.backgroundImage = `url("${country.flag}")`;
    fiteredDiv.innerHTML = `<p class="box-content">${country.name}</p>`;
    display.appendChild(fiteredDiv);
  });
...
function displayCountries() {
  ...
  countries_data.forEach((country) => {
    ...
    outer.className = "box1";
    outer.style.backgroundImage = `url("${country.flag}")`;
    outer.style.backgroundPosition = "center";
    outer.style.backgroundSize = "cover";
    outer.innerHTML = `<p class="box-content">${country.name}</p>`;
...
userEntry.addEventListener("input", () => {
  display.innerHTML = "";
  letterName.innerHTML = userEntry.value.toUpperCase();
  letterName.color = "black";
  const filtered = filterArr();
  countryQuantity.innerHTML = filtered.length;
});
